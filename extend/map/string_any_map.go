// Package exMap file from https://github.com/gogf/gf
package exMap
...
// NewStrAnyMapFrom creates and returns a hash map from given map <base>.
// Note that, the param <base> map will be set as the underlying base map(no deep copy),
...
// Clone returns a new hash map with copy of current map base.
func (m *StringAnyMap) Clone() *StringAnyMap {
	return NewStrAnyMapFrom(m.MapCopy())
}

// Map returns the underlying base map.
// Note that, if it's in concurrent-safe usage, it returns a copy of underlying base,
// or else a pointer to the underlying base.
...
// MapStrAny returns a copy of the underlying base of the map as map[string]facade{}.
func (m *StringAnyMap) MapStrAny() map[string]interface{} {
	return m.Map()
}

// MapCopy returns a copy of the underlying base of the hash map.
...
//
// When setting value, if <value> is type of <func() facade {}>,
...
// Clear deletes all base of the map, it will remake a new underlying base map.
...
// Replace the base of the map with given <base>.
...
// MarshalJSON implements the facade MarshalJSON for json.Marshal.
...
// UnmarshalJSON implements the facade UnmarshalJSON for json.Unmarshal.
