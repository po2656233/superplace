...
func (n *Node) Settings() face.ProfileJSON {
	return n.settings
}
...
func GetNodeWithConfig(config *Config, nodeId string) (face.INode, error) {
	nodeConfig := config.GetConfig("node")
	if nodeConfig.LastError() != nil {
		return nil, cerr.Error("`nodes` property not found in config file.")
	}
...
func LoadNode(nodeId string) (face.INode, error) {
	return GetNodeWithConfig(cfg.jsonConfig, nodeId)
}

func findNodeId(nodeId string, nodeIdJson face.ProfileJSON) bool {
